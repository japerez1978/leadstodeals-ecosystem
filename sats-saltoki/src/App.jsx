...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      if (user?.app_metadata?.tenant_id) tId = user.app_metadata.tenant_id;
      if (!tId && user?.user_metadata?.tenant_id) tId = user.user_metadata.tenant_id;

      // Intento 2: RPC (si la función existe y es pública)
      if (!tId) {
        try {
          const { data: rpcData } = await supabase.rpc("get_user_tenant_id");
          if (rpcData) tId = rpcData;
        } catch(e){}
      }
      ...
      if (error) showMsg("❌ Error cargando: "+error.message, 8000);
        ...
        const rows = data||[];
        setSats(rows.map(fromRow));
        // Intento 3: De los registros (si hay alguno)
        if (!tId && rows.length > 0 && rows[0].tenant_id) {
  ...
  const handleImport = (e) => {
    ...
    reader.onload = async (ev) => {
      try {
        ...
        const rows = imported.map(s => toRow(s, tenantId));
